...
// src/hacker.js
var VERSION = "0.3.0";
var STORAGE_KEY = "dsh-pretext:hacker";
var DEFAULT_CONFIG = {
  enabled: true,
  theme: "matrix",
  typewriter: false,
...
function parseCommand(line) {
  ...
  switch (name) {
    case "help":
    case "?":
      return { ok: true, name: "help", args };
    case "status":
      return { ok: true, name: "status", args };
    case "reset":
      return { ok: true, name: "reset", args };
    case "all":
      if (args.length !== 1 || !/^(on|off)$/i.test(args[0]))
        return { ok: false, error: "usage: all <on|off>" };
      return { ok: true, name: "all", args: [args[0].toLowerCase()] };
    case "theme":
      if (args.length !== 1 || !THEMES[args[0].toLowerCase()])
        return { ok: false, error: "usage: theme <matrix|amber|mono>" };
      return { ok: true, name: "theme", args: [args[0].toLowerCase()] };
    case "speed":
      if (args.length !== 1 || !/^\d+$/.test(args[0]))
        return { ok: false, error: "usage: speed <1-200>" };
    ...
    case "console":
      if (args.length !== 1 || !/^(on|off)$/i.test(args[0]))
        return { ok: false, error: "usage: " + name + " <on|off>" };
      return { ok: true, name: BOOL_CMDS[name], args: [args[0].toLowerCase()] };
    default:
      return { ok: false, error: "unknown command '" + name + "' (try 'help')" };
  }
}
function applyCommand(cfg, parsed) {
  if (!parsed.ok)
    return { cfg, reply: "ERR: " + parsed.error };
  var next = resolveConfig(cfg);
  switch (parsed.name) {
    case "help":
      return { cfg, reply: HELP_TEXT };
    case "status": {
      var lines = ["status:"];
      for (var k in DEFAULT_CONFIG)
        lines.push("  " + k + " = " + next[k]);
      return { cfg, reply: lines.join(`
`) };
    }
    case "reset":
    ...
    default:
      return { cfg, reply: "ERR: unhandled command" };
  }
}
var HELP_TEXT = [
...
function startTypewriter(cfgRef, ui) {
  ...
  function collectTextNodes(n, out) {
    ...
    for (var i = 0;i < n.childNodes.length; i++)
      collectTextNodes(n.childNodes[i], out);
    return out;
  }
  function revealTick(block) {
    ...
    if (!entry)
      return;
    entry.revealed += cfgRef.current.typewriterSpeed * 0.02;
  ...
  function handleTextNode(node) {
    ...
    if (el.closest("#dsh-hk-console-box, #dsh-hk-console, script, style, textarea, input"))
      return;
    var text = node.nodeValue || "";
    if (text.trim().length < cfgRef.current.typewriterMinLen)
      return;
    var block = el;
    while (block && block !== document.body && getComputedStyle(block).display === "inline")
    ...
    active.set(block, { mask, clone, w, revealed: 0 });
    revealTick(block);
  }
...
` : "") + s;
  ...
  field.addEventListener("keydown", function(e) {
    if (e.key === "Enter")
      submit();
    if (e.key === "Escape")
      box.classList.remove("dsh-hk-open");
  });
  print("dsh-pretext hacker console ready. type 'help'.");
  return { root, btn, box };
}
function initHacker(api) {
  if (!isBrowser()) {
    var ncfg = resolveConfig({});
    return {
      getConfig: function() {
        return resolveConfig(ncfg);
      },
      patchConfig: function(patch) {
        ncfg = resolveConfig(patch);
        return resolveConfig(ncfg);
      },
      execCommand: function(line) {
        return applyCommand(ncfg, parseCommand(line)).reply;
      },
      destroy: function() {}
    };
  }
  ...
  try {
    ...
    if (raw)
      cfg = resolveConfig(JSON.parse(raw));
  } catch (e) {}
  ...
  function applyUi(nextCfg) {
    ...
    persist();
    if (!consoleUi)
      consoleUi = buildConsole(api, cfgRef, applyUi);
    applyConfig(cfg, {
      rain,
      scan,
      caret,
    ...
    if (cfg.rain && stopRain === null)
      stopRain = startRain(rain);
    if (!cfg.rain && stopRain !== null) {
      stopRain();
      stopRain = null;
      rain.getContext("2d").clearRect(0, 0, rain.width, rain.height);
    }
    if (cfg.typewriter && stopTw === null)
      stopTw = startTypewriter(cfgRef, {});
    if (!cfg.typewriter && stopTw !== null) {
      stopTw();
      stopTw = null;
    }
  ...
  return {
    ...
    patchConfig: function(patch) {
      applyUi(resolveConfig(Object.assign({}, cfg, patch)));
      return resolveConfig(cfg);
    },
    execCommand: function(line) {
